...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def CSA(self, game_state):
        ...
        return {'min_td' : min_key[0], 'turrets_danger' : turret_danger_densities[min_key[0]]}
    ...
    def determine_attack_strategy(self, game_state):
        ...
        if playerMP >= threshold:
            ...
            # safest_deployment[1] < playerMP / 2 = viable to do the basic scout (CHEAP SHOT)
            if safest_deployment[1] < playerMP / 2 and not path_is_blocked:
                ...
                # not blocked and distinct undefended 
                gamelib.debug_write('ATTACKING ::: DEMOLISHER_ATTACK')
                self.attack_delay = 0
                ...
                # spawn intercepters
                locations = [[5, 8], [22, 8]]
                count = max(1, int(game_state.get_resource(MP, 1) / 8))
                self.build_exceptions = [[21, 9], [21, 10], [20, 10], [6, 9], [6, 10], [7, 10]]
                game_state.attempt_spawn(INTERCEPTOR, locations, count)
        elif game_state.get_resource(MP, 1) > 9:
            locations = [[5, 8], [22, 8]]
            self.build_exceptions = [[21, 9], [21, 10], [20, 10], [6, 9], [6, 10], [7, 10]]
            count = max(1, int(game_state.get_resource(MP, 1) / 8))
            game_state.attempt_spawn(INTERCEPTOR, locations, count)
