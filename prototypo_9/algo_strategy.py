    ...
    def determine_attack_strategy(self, game_state):
        ...
        if playerMP >= int(threshold):
            ...
            #     self.split_scout_attack(game_state, playerMP, target_region)
                ...
                if use_demolishers:
                    self.build_exceptions.append([5, 10])
                self.attack_delay = 0
                self.attack_status = SPLIT_SCOUT_ATTACK
                spawn_loc1 = [15, 1]   
                spawn_loc2 = [16, 2]
                ...
                if use_demolishers:
                    game_state.attempt_spawn(DEMOLISHER, [0, 13], count)
                ...
                self.attack_status = NO_ATTACK
                self.build_exceptions = [] 
            if enemyMP >= max(9, min(30, interceptor_check)) and self.attack_status == NO_ATTACK:
                # spawn intercepters
        ...
        elif enemyMP >= max(9, min(30, interceptor_check)) and self.attack_status == NO_ATTACK:
            # spawn intercepters
            locations = [[10, 3], [17, 3]]
            count = max(1, int(enemyMP / 12))
            game_state.attempt_spawn(WALL, [13, 1])
