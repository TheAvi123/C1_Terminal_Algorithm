    ...
    def determine_attack_strategy(self, game_state):
        ...
        if playerMP >= int(threshold):
            ...
            #     self.split_scout_attack(game_state, playerMP, target_region)
                ...
                if corner_structure is None or corner_structure == SUPPORT or (corner_structure == TURRET and corner_structure.upgraded is False):
                ...
                elif corner_structure.upgraded and playerMP >= 20:
                    gamelib.debug_write('ATTACKING ::: SNEAK_ATTACK DEFENDED')
                    count = int(playerMP * 3 / 4) + 1
                else:
                    gamelib.debug_write('ATTACKING ::: SNEAK_ATTACK NOT ATTACKING')
                    count = -1
                ...
                self.attack_status = NO_ATTACK
                self.build_exceptions = [] 
            if enemyMP >= max(9, min(30, interceptor_check)):
                # spawn intercepters
        ...
        elif enemyMP >= max(9, min(30, interceptor_check)):
            # spawn intercepters
            locations = [[8, 5], [19, 5]]
            count = max(1, int(enemyMP / 12))
            self.build_exceptions = [[6, 9]]
